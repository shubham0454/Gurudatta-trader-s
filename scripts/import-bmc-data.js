/**
 * Helper script to import BMC users JSON data
 * 
 * Usage:
 * 1. Paste your complete JSON array into 'bmc-users-data.json' in this directory
 * 2. Run: node scripts/import-bmc-data.js
 * 3. The script will convert and save it to 'bmc-users-data.json'
 */

const fs = require('fs')
const path = require('path')

const inputFile = path.join(__dirname, 'bmc-users-data.json')
const outputFile = path.join(__dirname, 'bmc-users-data.json')

try {
  // Check if input file exists
  if (!fs.existsSync(inputFile)) {
    console.log('❌ File not found: bmc-users-data.json')
    console.log('\n📝 Instructions:')
    console.log('1. Create a file called "bmc-users-data.json" in the scripts folder')
    console.log('2. Paste your complete JSON array into that file')
    console.log('3. Run this script again: node scripts/import-bmc-data.js')
    process.exit(1)
  }
  ...
  if (parsed.length > 0 && parsed[0].hasOwnProperty('id') && !parsed[0].hasOwnProperty('number_prefix')) {
    ...
    console.log(`\n🚀 You can now run: npm run seed:bmc`)
    process.exit(0)
  }
